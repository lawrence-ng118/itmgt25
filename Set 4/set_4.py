...
def bin_to_dec(binary_string):
    ...
    total = 0
    counter = 0
    for i in str(binary_string):
        total += (int(i) * (2**(len(str(binary_string))-1-counter)))
        counter += 1
...
def dec_to_bin(number):
    ...
    # figure out the max 2**smth that it can do for the number
    power = 0
    done_condition = 0
    binary_rep = 1
    final =[]
    while done_condition == 0:
        if number > binary_rep:
            power += 1
        elif number < binary_rep:
            if power != 0:
                power -= 1
            done_condition += 1
        elif number == binary_rep:
    ...
    counter = 0
    while (power-counter) != -1:
        if number >= (2**(power-counter)):
            number -= (2**(power-counter))
            final.append("1")
        elif number < (2**(power-counter)):
            final.append("0")
        counter += 1
...
def telephone_cipher(message):
    ...
    for j in message_split:
        message_split[index] = encoder_dict[j]
        if index != 0:
            if message_split[index][0] == message_split[index-1][0]:
                message_split[index-1] += "_"
        index += 1
...
def telephone_decipher(telephone_string):
    ...
    the_message_in_numbers = []
    telephone_string += "_"
    index1 = 0
    index2 = 0

    while index2 <= (len(telephone_string)):
        #if index2 == (len(telephone_string)-1):
            #the_message_in_numbers.append(telephone_string[index1:index2+1])
        if telephone_string[index2] != telephone_string[index1]:
            the_message_in_numbers.append(telephone_string[index1:index2])
            if telephone_string[index2] == "_":
                index2 += 1
            index1 = index2
        index2 += 1
    #if telephone_string[-1] == the_message_in_numbers[-1][0]:
            #the_message_in_numbers[-1] += telephone_string[-1]
    index = 0
    for j in the_message_in_numbers:
        the_message_in_numbers[index] = decipher_dict[j]
        index += 1
    
    return "".join(the_message_in_numbers)