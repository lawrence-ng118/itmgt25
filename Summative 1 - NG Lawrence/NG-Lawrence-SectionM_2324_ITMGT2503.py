...
for i in range(0, len(jsondata)):
    before_string = str(jsondata[str(i)])
    print(before_string)
    print(type(before_string))
    string = before_string.upper()
...
for i in range(0, len(jsondata)):
    sentence_split = jsondata[str(i)].split()
    for j in sentence_split:
        if len(j) >= 3:
            if j in wordcount_dictionary.keys():                
                wordcount_dictionary[j] += 1
            elif j not in wordcount_dictionary.keys():
                wordcount_dictionary[j] = 1
...
# Hint: Don't use your DecSci brain, use your Python programming brain = 0
storage = [0]
for F in range(0, min(18000//1000, 12000//500, 1500//50)+1):
    for R in range(0, min(18000//600, 12000//750, 1500//45)+1):
        for C in range(0, min(18000//750, 12000//300, 1500//75)+1):
            if ((1000*F + 600*R + 750*C) <= 18000) & ((500*F + 750*R + 300*C) <= 12000) & ((50*F + 45*R + 75*C) <= 1500):
                Z = 10*F + 12*R + 16*C
                if Z > storage[0]:
                    storage[0] = Z
                    optimal_Z = Z
...
def Collatz(start_number):
    ...
    while 1 not in collatz_sequence:
        if collatz_sequence[-1]%2 == 0:
            collatz_sequence.append(int(collatz_sequence[-1]/2))
        elif collatz_sequence[-1]%2 != 0:
            collatz_sequence.append(int(collatz_sequence[-1]*3 + 1))

    my_dict = {}
    my_dict["collatz_sequence"] = collatz_sequence
    my_dict["collatz_length"] = len(collatz_sequence)
    my_dict["max_collatz"] = max(collatz_sequence)   
    my_dict["sequence_sum"] = int(sum(collatz_sequence))
...
def Collatz_winner(number_list):
    ...
    for i in number_list:
        collatz_data.append(Collatz(i))

    for j in range(0, len(collatz_data)):
        counter_for_winner = 0
        for k in range(0, len(collatz_data)):
            if k != j:
                if (collatz_data[j]["collatz_length"] >= collatz_data[k]["collatz_length"]) & (collatz_data[j]["max_collatz"] >= collatz_data[k]["max_collatz"]):
                    counter_for_winner += 1
        if counter_for_winner == len(collatz_data)-1:
            return collatz_data[j]["collatz_sequence"][0]
